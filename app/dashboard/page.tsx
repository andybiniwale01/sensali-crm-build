...
import { currentAgent, recentListing, getGreeting, formatDate, getWeekOf, getLastUpdated, Property } from '@/lib/data'
...
export default function DashboardPage() {
  ...
  const stats = {
    totalScored:  properties.length,
    top20:        Math.min(20, properties.length),
    hotLeads:     properties.filter(p => p.status === 'HOT').length,
    listingsWon:  properties.filter(p => p.pipelineStage === 'LISTING_WON').length,
  }
      ...
      <main className="ml-64 min-h-screen">
        {showListingBanner && (
          <div className="bg-[#C9A84C]/10 border-b border-[#C9A84C]/30 px-6 py-3 flex items-center justify-between">
            ...
            <button onClick={() => setShowListingBanner(false)} className="text-[#8b949e] hover:text-white">
        ...
        <div className="p-8">
          <div className="flex justify-between items-start mb-8">
            <div>
              <h1 className="text-2xl font-bold text-white">{getGreeting()}, {currentAgent.name.split(' ')[0]}</h1>
          ...
          <div className="grid grid-cols-4 gap-4 mb-8">
            <StatCard title="Total Scored" value={loading ? '...' : stats.totalScored}
              icon={<svg className="w-6 h-6" fill="none" stroke="currentColor" viewBox="0 0 24 24"><path strokeLinecap="square" strokeWidth={1.5} d="M9 19v-6a2 2 0 00-2-2H5a2 2 0 00-2 2v6a2 2 0 002 2h2a2 2 0 002-2zm0 0V9a2 2 0 012-2h2a2 2 0 012 2v10m-6 0a2 2 0 002 2h2a2 2 0 002-2m0 0V5a2 2 0 012-2h2a2 2 0 012 2v14a2 2 0 01-2 2h-2a2 2 0 01-2-2z" /></svg>}
            />
            <StatCard title="This Week's Top 20" value={loading ? '...' : stats.top20}
              icon={<svg className="w-6 h-6" fill="none" stroke="currentColor" viewBox="0 0 24 24"><path strokeLinecap="square" strokeWidth={1.5} d="M13 7h8m0 0v8m0-8l-8 8-4-4-6 6" /></svg>}
            />
            <StatCard title="Hot Leads" value={loading ? '...' : stats.hotLeads}
              icon={<svg className="w-6 h-6" fill="none" stroke="currentColor" viewBox="0 0 24 24"><path strokeLinecap="square" strokeWidth={1.5} d="M17.657 18.657A8 8 0 016.343 7.343S7 9 9 10c0-2 .5-5 2.986-7C14 5 16.09 5.777 17.656 7.343A7.975 7.975 0 0120 13a7.975 7.975 0 01-2.343 5.657z" /><path strokeLinecap="square" strokeWidth={1.5} d="M9.879 16.121A3 3 0 1012.015 11L11 14H9c0 .768.293 1.536.879 2.121z" /></svg>}
            />
            <StatCard title="Listings Won" value={loading ? '...' : stats.listingsWon} icon={<TrophyIcon />} />
          </div>
          ...
          <div className="bg-[#161b22] border border-[#30363d]">
            ...
            <div className="overflow-x-auto">
              <table className="w-full">
                ...
                <tbody>
                  {loading ? (
                    <tr><td colSpan={7} className="px-4 py-12 text-center text-[#8b949e]">Loading properties...</td></tr>
                  ) : top20.map((property) => (
                    <tr
                      key={property.id}
                    ...
                    >
                      <td className="px-4 py-4"><span className="text-2xl font-bold text-[#C9A84C]">{property.rank}</span></td>
